  ...
  return (
    ...
    >
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute top-1/2 left-0 w-1/2 h-1/2 bg-gradient-to-r from-[#3b82f6]/5 via-transparent to-transparent rounded-full blur-3xl" />
      </div>
      ...
      >
        <div className="text-center mb-16 md:mb-20">
          <div className="flex items-center justify-center mb-4">
            <Award className="w-8 h-8 text-[#3b82f6] mr-3" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {certifications.map((cert, index) => (
            <motion.a
              ...
              viewport={{ once: true }}
            >
              <Card className="group relative overflow-hidden bg-gradient-to-br from-[#3b82f6]/8 to-[#9d7f6f]/5 border border-[#3b82f6]/30 bg-[#1a1f2e]/50 backdrop-blur-sm hover:border-[#3b82f6]/60 transition-all duration-300 cursor-pointer hover:shadow-xl hover:shadow-[#3b82f6]/15 hover:-translate-y-1 flex flex-col h-full">
                ...
                <div className="relative w-full h-40 overflow-hidden border-b border-[#3b82f6]/30">
                  <Image
                    src={`/${cert.image}`}
                    alt={cert.title}
                    fill
                ...
                <div className="relative z-10 p-6 flex flex-col flex-grow">
                  <h3 className="text-lg font-bold text-[#f5f3f0] mb-2 line-clamp-2 group-hover:text-[#3b82f6] transition-colors duration-300">
                    {cert.title}
                  </h3>

                  <p className="text-sm text-[#3b82f6] font-semibold mb-1">{cert.issuer}</p>
                  <p className="text-xs text-[#f5f3f0]/50 mb-4">{cert.date}</p>

                  <p className="text-sm text-[#f5f3f0]/60 leading-relaxed flex-grow">{cert.description}</p>

                  <div className="mt-4 h-1 bg-gradient-to-r from-[#3b82f6] to-[#9d7f6f] group-hover:opacity-100 opacity-60 transition-all duration-300" />
                </div>
              </Card>
            </motion.a>
          ))}
